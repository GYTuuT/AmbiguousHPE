...
class Flow(Distribution):
    ...
    def sample_and_log_prob(self, num_samples, noise=None, context=None, ignore_context=False):
        ...
        if noise is None:
            ...
            log_prob = self._distribution.log_prob(
                inputs=noise.reshape(batch_size * num_samples, -1), 
                context=embedded_context.unsqueeze(1).repeat(1, num_samples, 1).reshape(batch_size * num_samples, -1) if context is not None else None
            )
    ...
    def sample_log_prob(self, noise, context=None, ignore_context=False):
        """Generates samples from the flow, together with their log probabilities.
